...
PROTON_PATH=$(find / type -d -path "*/compatdata/9480/pfx" -print -quit 2> /dev/null)

# Check to see if the settings folder is located in AppData or Local Settings.
if ! [[ $(ls "$PROTON_PATH/drive_c/users/steamuser/AppData/Local/THQ/Saints Row 2/") ]]; then
	PATH_TO_SETTINGS="$PROTON_PATH/drive_c/users/steamuser/Local Settings/Application Data/THQ/Saints Row 2/"
elif [[ $(ls "$PROTON_PATH/drive_c/users/steamuser/AppData/Local/THQ/Saints Row 2/") ]]; then
	PATH_TO_SETTINGS="$PROTON_PATH/drive_c/users/steamuser/AppData/Local/THQ/Saints Row 2/"
else
	echo "Cannot locate 'settings.dat', are you sure \"$PROTON_PATH\" is your prefix path?"
	exit 1
fi
